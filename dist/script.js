        ...
        switch (item.nodeName) {
          ...
          case 'INPUT':
            if (item.getAttribute('type') === 'checkbox') {
              i === 0 ? state[prop] = 'Холодный' : state[prop] = 'Теплое';
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault();
      var statusMesssage = document.createElement('div');
      ...
      postData('assets/server.php', formData).then(function (res) {
        ...
        return statusMesssage.textContent = message.failure;
      }).finally(function () {
        clearInput();
        setTimeout(function () {
